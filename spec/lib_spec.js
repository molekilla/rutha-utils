...
describe("Lib Test", function () {
    it("should throw missing feature toggle section error", function () {
        process.env.NODE_ENV = 'development';
        expect(function () {
                config.isFeatureEnabled('api');
            })
            .toThrow(new Error('Missing _featureToggles config section'));
    });
    ...
    it("should load feature toggle", function () {
        process.env.NODE_ENV = 'test';
        var test = config.load();
        expect(test.isFeatureEnabled('api')).toBe(true);
    });
